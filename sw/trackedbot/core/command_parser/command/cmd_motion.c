 ...
 *
 * @brief Implementation of CMD_ID_MOTION (0x01) handler.
 */
...
bool cmd_motion_execute(
  ...
  if (len != (uint8_t)PAYLOAD_LEN_MOTION || payload == (void *)0) {
    if (eh != (void *)0) {
  ...
  uint8_t m_cmd = payload[0];
  if (m_cmd > (uint8_t)MOTION_SPIN_RIGHT) {
    if (eh != (void *)0) {
      (void)error_handler_report(eh, ERR_CODE_CMD_SYNTAX_ERROR);
    }
    return telemetry_reporter_send_nack(stream, msg_id, ERR_CODE_CMD_SYNTAX_ERROR);
  ...
  (void)tracked_motion_set_command(motion, (motion_cmd_t)m_cmd);
