...
motion_status_t slew_limiter_init(slew_limiter_t *limiter, uint8_t initial_val, uint8_t rate_per_step) {
  if (limiter == (void *)0 || rate_per_step == 0) {
    return MOTION_STATUS_INVALID_ARG;
  }

  limiter->current_val = initial_val;
  limiter->target_val = initial_val;
  limiter->rate_per_step = rate_per_step;
...
motion_status_t slew_limiter_update(slew_limiter_t *limiter) {
  ...
  if (limiter->current_val < limiter->target_val) {
    uint8_t diff = (uint8_t)(limiter->target_val - limiter->current_val);
    if (diff <= limiter->rate_per_step) {
      limiter->current_val = limiter->target_val;
    } else {
      limiter->current_val = (uint8_t)(limiter->current_val + limiter->rate_per_step);
    }
  } else if (limiter->current_val > limiter->target_val) {
    uint8_t diff = (uint8_t)(limiter->current_val - limiter->target_val);
    if (diff <= limiter->rate_per_step) {
      limiter->current_val = limiter->target_val;
    } else {
      limiter->current_val = (uint8_t)(limiter->current_val - limiter->rate_per_step);
...
bool slew_limiter_is_finished(const slew_limiter_t *limiter) {
  ...
  return (limiter->current_val == limiter->target_val);
}
...
motion_status_t slew_limiter_reset(slew_limiter_t *limiter, uint8_t value) {
  ...
  limiter->current_val = value;
  limiter->target_val = value;
